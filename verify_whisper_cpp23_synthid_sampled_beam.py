...
pattern = re.compile(r"synthid_state_rows=(\d+) synthid_hashes=([^ ]*) synthid_repeated=([01]*) synthid_skipped=([01]*) graph_nodes_visited=(\d+)$")
def run():
    ...
    assert rows % 2 == 0 and int(match.group(5)) == 74
    artifact = {"certificate": "WHISPER_CPP23_SAMPLED_BEAM_SYNTHID_1",
                ...
                "graph_nodes_visited": int(match.group(5)),
    ...
    (OUT / "WHISPER_CPP23_SYNTHID_SAMPLED_BEAM.md").write_text(
        ...
        "The exact explicit C++ seed replayed identically. This is not a claim of identical PyTorch/C++ random bitstreams.\n")
