...
cache_implementations = json.loads(
    Path("outputs/whisper_cpp23_cache_implementations.json").read_text()
)
portable_model = json.loads(
    Path("outputs/whisper_cpp23_portable_model.json").read_text()
)
deadline = json.loads(Path("outputs/whisper_cpp23_deadline.json").read_text())
...
requirements = [
    ...
    {
        "requirement": "monotonic generation deadline state",
        "status": "PROVED_FINITE_ALL_CONVERTED_SEARCHES",
        "evidence": f"{deadline['transition_case_count']} strict-boundary transitions, {deadline['generation_case_count']} greedy runs, and {deadline['search_mode_case_count']} search-mode finalizations agree with pinned references; all runs visit 74 nodes",
    },
    {
    ...
    },
    {
        ...
        "evidence": f"{model_applicability['rejected_case_count']} model-rejected and {model_applicability['ignored_case_count']} model-ignored cases agree with pinned Transformers, including three non-default BOS values; DoLa source hash pinned",
    },
    {
        "requirement": "public generation cache representation",
        "status": "PROVED_FINITE_OUTPUT_PROJECTION",
        "evidence": f"EncoderDecoderCache and legacy four-tuple projections are byte-identical in C++23, sequences match, and both compare with PyTorch within {cache_projection['worst_max_absolute_cache_error']:.9g}",
    },
    {
        "requirement": "generation cache allocation and prefill applicability",
        "status": "PROVED_FINITE_CPU_AND_SOURCE_STRUCTURAL",
        "evidence": f"{cache_implementations['executable_case_count']} cache requests preserve exact greedy sequences and bitwise-equal within-runtime logical tensors; dynamic/static policy preserves tokens across {cache_implementations['search_mode_case_count']} converted searches with explicit contrastive/assisted dynamic overrides; {cache_implementations['rejected_case_count']} cache requests reproduce pinned rejection categories; {cache_implementations['prefill_policy_case_count']} prefill cases preserve {cache_implementations['prefill_rejected_case_count']} rejection and {cache_implementations['prefill_ignored_case_count']} ignored-mode outcomes; worst C++23/PyTorch cache error {cache_implementations['worst_max_absolute_cache_error']:.9g}",
    },
    {
...
out = {
    ...
    "cache_projection_fixture_sha256": sha(
        "outputs/whisper_cpp23_cache_projection.json"
    ),
    "cache_projection_values_bitwise_equal": cache_projection[
    ...
    "cache_implementations_fixture_sha256": sha(
        "outputs/whisper_cpp23_cache_implementations.json"
    ),
    "cache_implementation_executable_cases": cache_implementations[
        "executable_case_count"
    ],
    "cache_implementation_rejected_cases": cache_implementations[
        "rejected_case_count"
    ],
    "prefill_policy_cases": cache_implementations["prefill_policy_case_count"],
    "prefill_rejected_cases": cache_implementations[
        "prefill_rejected_case_count"
    ],
    "prefill_ignored_cases": cache_implementations[
        "prefill_ignored_case_count"
    ],
    "cache_implementation_storage_values_bitwise_equal": cache_implementations[
        "all_cpp23_storage_values_bitwise_equal"
    ],
    "cache_search_mode_cases": cache_implementations["search_mode_case_count"],
    "cache_search_mode_tokens_storage_invariant": cache_implementations[
        "all_search_mode_tokens_storage_invariant"
    ],
    "cache_standard_beam_static_tokens_exact": cache_implementations[
        "beam_static_tokens_exact"
    ],
    "cache_sampled_beam_static_tokens_exact": cache_implementations[
        "sampled_beam_static_tokens_exact"
    ],
    "cache_contrastive_forced_dynamic_full": cache_implementations[
        "contrastive_cache_forced_dynamic_full"
    ],
    "cache_assisted_forced_dynamic": cache_implementations[
    ...
    "deadline_fixture_sha256": sha("outputs/whisper_cpp23_deadline.json"),
    "deadline_transition_cases": deadline["transition_case_count"],
    "deadline_generation_cases": deadline["generation_case_count"],
    "deadline_search_mode_cases": deadline["search_mode_case_count"],
    "deadline_all_generation_tokens_exact": deadline[
        "all_generation_tokens_exact"
    ],
    "deadline_all_search_mode_tokens_exact": deadline[
